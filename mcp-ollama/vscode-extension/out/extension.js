...
const inlineSuggestionProvider_1 = require("./inlineSuggestionProvider");
const copilotChatProvider_1 = require("./copilotChatProvider");
const streamingClient_1 = require("./streamingClient");
const workspaceAnalyzer_1 = require("./workspaceAnalyzer");
const copilotUI_1 = require("./copilotUI");
...
async function activate(context) {
    ...
    try {
        ...
        suggestionProvider = new inlineSuggestionProvider_1.InlineSuggestionProvider(mcpClient);
        chatProvider = new copilotChatProvider_1.CopilotChatProvider(mcpClient);
        streamingClient = new streamingClient_1.StreamingClient(mcpClient);
        workspaceAnalyzer = new workspaceAnalyzer_1.WorkspaceAnalyzer(mcpClient);
        copilotUI = new copilotUI_1.CopilotUI(mcpClient);
        // Register inline completion provider
        const completionProvider = vscode.languages.registerInlineCompletionItemProvider({ pattern: '**' }, suggestionProvider);
        // Register chat provider with proper error handling
        let chatProviderRegistration;
        try {
            chatProviderRegistration = vscode.chat.createChatParticipant('smartcode-aiassist', chatProvider.handleChatRequest.bind(chatProvider));
        ...
        try {
            if (chatProvider) {
                copilotChatProvider_1.CopilotChatProvider.registerChatCommands(context, mcpClient);
            }
        }
        catch (error) {
...
function getTestFileName(originalFileName, language) {
...
async function initializeMCPConnection(retries = 3) {
    if (!mcpClient) {
